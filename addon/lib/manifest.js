...
export function manifest(config = {}) {
  ...
  const baseManifest = {
    id: 'com.stremio.ultimatestream.addon',
    version: '0.0.14',
...
function getName(manifest, config) {
  const rootName = manifest?.name || 'UltimateStream';
...
function getDescription(config) {
  ...
  return 'Provides torrent streams from scraped torrent providers.'
      ...
      + ` To configure providers, ${possibleMochs} support and other settings visit https://ultimatestream.strem.fun`
}
...
function getCatalogs(config) {
  return MochProviders
      ...
      .map(moch => moch.catalogs.map(catalogName => ({
        id: catalogName ? `ultimatestream-${moch.key}-${catalogName.toLowerCase()}` : `ultimatestream-${moch.key}`,
